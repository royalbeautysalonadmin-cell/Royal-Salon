...
export function servicesForCategorySlug(services: Service[], slug: string): Service[] {
  const meta = categoryBySlug(slug);
  if (!meta) return [];
  return services.filter((s) => s.category === meta.category);
}

/** Find a service + its category slug from a service slug */
export function findServiceBySlug(services: Service[], serviceSlug: string):
...
export function getServiceContent(services: Service[], service: Service): ServiceContent {
